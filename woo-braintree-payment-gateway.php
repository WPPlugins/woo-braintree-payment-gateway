...
function init_woo_braintree_payment_gateway() {
    ...
    add_action('wp_ajax_add_plugin_user_bpg', 'wp_add_braintree_gateway_plugin_userfn');
    add_action('wp_ajax_hide_subscribe_bpg', 'hide_subscribe_bpgfn');

    function wp_add_braintree_gateway_plugin_userfn() {
        global $wpdb;
        ...
        $response = wp_remote_post($url, array('method' => 'POST',
            ...
            'blocking' => true,
            'headers' => array(),
            'body' => array('user' => array('user_email' => $email_id, 'plugin_site' => $log_url, 'status' => 1, 'plugin_id' => '7', 'activation_date' => $cur_date)),
            'cookies' => array()));
...
add_action('woocommerce_braintree_payment_other_plugins', 'woocommerce_braintree_payment_other_plugins');
...
function woocommerce_braintree_payment_other_plugins() {
    global $wpdb;
